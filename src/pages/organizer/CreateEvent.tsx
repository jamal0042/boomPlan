...
// Définition des interfaces de données pour le formulaire d'événement
...
// Définition des interfaces de données pour le formulaire de billet
...
const CreateEvent: React.FC = () => {
  ...
  // États pour le formulaire en plusieurs étapes
  const [currentStep, setCurrentStep] = useState(1);
  const [tickets, setTickets] = useState<TicketFormData[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isUploadingImage, setIsUploadingImage] = useState(false);

  // États pour l'upload de l'image
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(null);

  // Utilisation de useForm pour le formulaire d'événement
  const {
  ...
  // Utilisation de useForm pour le formulaire de billets
  const {
  ...
  // Récupération des valeurs des dates de début et de fin pour la validation croisée
  const startDatetime = watch('start_datetime');
  const endDatetime = watch('end_datetime');

  // Fonction pour gérer la sélection d'un fichier image et sa prévisualisation
  ...
  const addTicket: SubmitHandler<TicketFormData> = (data) => {
  ...
  // Fonction pour la soumission finale de l'événement
  const onEventSubmit: SubmitHandler<EventFormData> = async (data) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (imageFile) {
        ...
        // Ici, vous enverriez le fichier au serveur. Simulé pour l'exemple.
        const uploadedImageUrl = await new Promise<string>((resolve) => {
          setTimeout(() => {
            console.log("Simulating image upload...");
            resolve('https://placehold.co/800x400/E5E7EB/4B5563?text=Image+de+l\'événement');
          }, 1500);
        });
      ...
      // Étape 2: Création de l'événement dans le backend
  ...
  // Données pour les étapes de progression
  const steps = [
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-sm">
          ...
          {currentStep === 1 && (
            <form
              onSubmit={handleEventSubmit(() => setCurrentStep(2), (errors) => console.log(errors))}
            ...
            >
              ...
              <div>
                ...
                <input
                  id="title"
                  {...registerEvent('title', { required: 'Le titre est requis' })}
                  type="text"
              ...
              <div>
                ...
                {eventErrors.category_id && (
                  <p className="mt-1 text-sm text-red-600">{eventErrors.category_id.message}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    id="location"
                    {...registerEvent('location', { required: 'Le nom du lieu est requis' })}
                    type="text"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              {/* Date et heure avec validation croisée */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    id="start_datetime"
                    {...registerEvent('start_datetime', {
                      required: 'La date de début est requise',
                      validate: (value) =>
                        new Date(value) > new Date() || 'La date de début doit être future',
                    })}
                    type="datetime-local"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                ...
                <div>
                  ...
                  <input
                    id="end_datetime"
                    {...registerEvent('end_datetime', {
                      required: 'La date de fin est requise',
                      validate: (value) =>
                        !startDatetime || new Date(value) > new Date(startDatetime) || 'La date de fin doit être après la date de début',
                    })}
                    type="datetime-local"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                      ...
                      <div key={index} className="flex justify-between items-center p-3 border border-gray-200 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            ${(ticket.price || 0).toFixed(2)} • {ticket.quantity_total} disponibles
                    ...
                    <div className="space-y-2">
                      {tickets.map((ticket, index) => (
                        <div key={index} className="flex justify-between text-sm">
                          ...
                          <span>${(ticket.price || 0).toFixed(2)} × {ticket.quantity_total}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  >
                    {isLoading || isUploadingImage ? (
                      <div className="flex items-center">
                        ...
                        {isUploadingImage ? "Téléchargement de l'image..." : "Création de l'événement..."}
                      </div>
                    ) : (
                      'Créer l\'événement'
                    )}
